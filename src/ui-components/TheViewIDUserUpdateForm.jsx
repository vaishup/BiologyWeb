...
import { getTheViewIDUser } from "../graphql/queries";
import { updateTheViewIDUser } from "../graphql/mutations";
const client = generateClient();
export default function TheViewIDUserUpdateForm(props) {
  const {
    id: idProp,
    theViewIDUser: theViewIDUserModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = theViewIDUserRecord
      ? { ...initialValues, ...theViewIDUserRecord }
      : initialValues;
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getTheViewIDUser.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTheViewIDUser
        : theViewIDUserModelProp;
      setTheViewIDUserRecord(record);
    };
    queryData();
  }, [idProp, theViewIDUserModelProp]);
  React.useEffect(resetStateValues, [theViewIDUserRecord]);
  const validations = {
    name: [],
    employeeId: [],
    profileStatus: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateTheViewIDUser.replaceAll("__typename", ""),
            variables: {
              input: {
                id: theViewIDUserRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "TheViewIDUserUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || theViewIDUserModelProp)}
